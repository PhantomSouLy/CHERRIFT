/* CHERRIFT network guard
 * Canonical owner: browser fetch, Supabase client, Auth lock and Edge Function
 * timeouts. Optional artwork warmup is owned by cherrift_app.js.
 */
(() => {
  "use strict";
  ...
  window.__CHERRIFT_NETWORK_GUARD__ = Object.freeze({
    ...
    preload:"runtime-owned"
  });

  /* Supabase client singleton, bounded Web Lock, Auth and Edge Function calls.
   * Kept in this network owner so the browser has one canonical timeout layer.
  ...
  window.__CHERRIFT_SUPABASE_GUARD_INSTALLED__ = true;
  ...
  function makeSingletonFactory(factory, owner = null) {
    ...
    markFunction(wrapped, "__CHERRIFT_SINGLETON_FACTORY__");
    return wrapped;
  }
  ...
  console.info(
    `[CHERRIFT] Network guard ${VERSION} active. ` +
