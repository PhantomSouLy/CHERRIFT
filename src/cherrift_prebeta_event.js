(() => {
  ...
  function hidePanel(){const panel=$("eventHubPrebeta");if(!panel)return;panel.classList.add("hidden");panel.style.display="none";panel.setAttribute("aria-hidden","true");document.body.classList.remove("prebeta-event-open")}

  function close(){window.UI?.open?.("menu");hidePanel();q("#menu")?.classList.remove("hidden");document.body.classList.remove("is-playing","is-levelup","is-loading-stage");window.dispatchEvent(new CustomEvent("cherrift:prebeta-event-close"))}

  function open(tab=state.tab){state.tab=tab;ensureSave();const panel=ensurePanel();window.UI?.open?.("menu");q("#menu")?.classList.add("hidden");document.querySelectorAll("#app > .panel").forEach(node=>node.classList.add("hidden"));panel.classList.remove("hidden");panel.style.removeProperty("display");panel.setAttribute("aria-hidden","false");document.body.classList.add("prebeta-event-open");document.body.classList.remove("is-playing","is-levelup","is-loading-stage");render();persist()}
