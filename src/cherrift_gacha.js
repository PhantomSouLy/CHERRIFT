(() => {
  ...
  const VERSION = "2.2.0-routed-equipment-chests";
  ...
  const COPY = Object.freeze({
    hu: {
      title:"Felszerelés ládák", back:"Vissza", guaranteed:"Skin garancia", opened:"Kinyitva", empty:name => `Nincs ${name}.`,
    ...
    en: {
      title:"Equipment Chests", back:"Back", guaranteed:"Skin Guarantee", opened:"Opened", empty:name => `You don't have any ${name}s.`,
  ...
  function ensureCss() {
    ...
    style.textContent = `
      ...
      body.gacha-open :is(.resource-bar-v082,#resourceBarV082,#globalRailV060,#desktopCurrencyV0943,.mobile-header-v0932,.mobile-currencies-v0932,.economy-wallet-v080){display:none!important;visibility:hidden!important;pointer-events:none!important}
      #gachaChestOnlyV12{overflow:hidden!important;overscroll-behavior:none;touch-action:pan-y;min-height:var(--cherrift-viewport-height,100dvh);color:#fff}
      ...
      .gco-rarity{margin:20px 0 3px;font-weight:1000;letter-spacing:4px;text-transform:uppercase}.gco-card.common .gco-rarity{color:#63dd8a}.gco-card.rare .gco-rarity{color:#58adff}.gco-card.epic .gco-rarity{color:#c060ff}.gco-card h3{margin:0;font:700 clamp(38px,7vw,58px)/1.05 Georgia,serif}.gco-copy{margin:18px 0 8px;color:#e7c9db;font-size:18px}.gco-empty-note{margin:0 0 14px;padding:9px 12px;border:1px solid #ffb4d13d;border-radius:12px;color:#ffb4d1;background:#b9276414;font-weight:850}.gco-pity{padding:15px 18px;border-radius:18px;background:#ffffff07}.gco-pity header{display:flex;justify-content:space-between;font-size:19px}.gco-pity-meta{margin-top:8px;color:#d9b8ce;font-size:12px;font-weight:850}.gco-track{height:10px;margin-top:10px;border-radius:99px;background:#ffffff0d;overflow:hidden}.gco-track i{display:block;height:100%;background:linear-gradient(90deg,#e34b98,#b65cff)}
  ...
  function ensurePanel() {
    ...
    panel.innerHTML = `<div class="gco-shell"><header class="gco-head"><button class="gco-back" type="button" data-gco-back aria-label="${esc(text("back"))}">←</button><h2 data-gco-title>${esc(text("title"))}</h2></header><div id="gcoChestWallet" class="gco-chests" aria-label="Chests"></div><div class="gco-carousel" id="gcoCarousel"><button class="gco-arrow" type="button" data-gco-step="-1" aria-label="${esc(text("previous"))}">‹</button><div id="gcoCard"></div><button class="gco-arrow" type="button" data-gco-step="1" aria-label="${esc(text("next"))}">›</button></div><div id="gcoDots" class="gco-dots"></div></div><div id="gcoModal" class="gco-modal hidden" role="dialog" aria-modal="true"></div><div id="gcoToast" class="gco-toast" role="status" aria-live="polite"></div>`;
  ...
  function openMany(amount) {
    ...
    // Gacha owns its reveal sequence, so persisting the roll never invokes
    // the separate Obtained acquisition UI.
    saveProgress(save);
    render();
