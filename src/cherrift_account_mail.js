(() => {
  ...
  const COPY = {
    hu: {
      ...
      activeTitleStats:"Aktív Title", ownedTitleBonuses:"Megszerezett Title bónuszok", titleCoinScope:"A Bónusz Coin kizárólag a pályán felszedett Coinra és a normál pályajutalomra érvényes. First Clear, login, láda, event és más menüjutalom nem kapja meg."
    },
    en: {
      ...
      activeTitleStats:"Active Title", ownedTitleBonuses:"Owned Title bonuses", titleCoinScope:"Bonus Coin applies only to Coin picked up during a stage and the normal stage reward. First Clear, login, chest, event and other menu rewards are excluded."
    }
  };
  ...
  function openTitleStatsModal() {
    ...
    if (!titles.length) {
      ...
      root.innerHTML = `<section class="title-stats-active-v096"><small>${esc(t("activeTitleStats"))}</small><h4>${esc(activeTitleName())}</h4><div class="title-stat-chips-v096">${active?.stats?.length?active.stats.map(stat=>`<span><b>${esc(stat.label)}</b><em>${esc(formatTitleStat(stat))}</em></span>`).join(""):`<span class="empty">${esc(t("titleStatsEmpty"))}</span>`}</div></section>
        <section class="title-stats-total-bf"><small>${esc(t("totalTitleStats"))}</small><div class="title-stat-chips-v096">${totalValues.length?totalValues.map(stat=>`<span><b>${esc(stat.label)}</b><em>${esc(formatTitleStat(stat))}</em></span>`).join(""):`<span class="empty">—</span>`}</div>${totalValues.some(stat=>stat.key==="coinGain")?`<p class="title-coin-scope-v096">ⓘ ${esc(t("titleCoinScope"))}</p>`:""}</section>
