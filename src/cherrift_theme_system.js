...
function createPetalBurst(x, y, options = {}) {
  ...
  const amount = Number(options.amount) || (lightweight ? 10 : 18);
  const fragment = document.createDocumentFragment();
  createPetalRipple(layer, x, y);
  for (let index = 0; index < amount; index++) {
    const petal = document.createElement("i");
    const angle = (Math.PI * 2 * index / amount) + (Math.random() - .5) * .52;
    const arc = lightweight ? 30 : 38;
    const burst = arc + Math.random() * (lightweight ? 26 : 38);
    const lift = 10 + Math.random() * 16;
    const drift = Math.cos(angle) * burst;
    const fall = Math.sin(angle) * burst + 28 + Math.random() * 38;
    petal.style.left = `${x}px`;
    petal.style.top = `${y}px`;
    petal.style.setProperty("--petal-x", `${drift.toFixed(1)}px`);
    petal.style.setProperty("--petal-y", `${fall.toFixed(1)}px`);
    petal.style.setProperty("--petal-lift", `${lift.toFixed(1)}px`);
    petal.style.setProperty("--petal-r", `${(Math.random() * 320 - 160).toFixed(0)}deg`);
    petal.style.setProperty("--petal-delay", `${Math.random() * 60}ms`);
    petal.style.setProperty("--petal-scale", `${(0.84 + Math.random() * 0.46).toFixed(2)}`);
    fragment.appendChild(petal);
    window.setTimeout(() => petal.remove(), lightweight ? 1020 : 1180);
  }
