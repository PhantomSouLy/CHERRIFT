...
function subnavItems(group) {
  const maps = {
    play:[
      ["socialV082", copy("Socials", "Socials")],
      ["eventV093", copy("Event", "Event")],
...
function ensureDesktopMenuButton(railNav) {
  let menuButton = railNav.querySelector('[data-v082-route="menu"]');
  if (!menuButton) {
    menuButton = document.createElement("button");
    menuButton.type = "button";
    menuButton.dataset.v082Open = "menu";
    menuButton.dataset.v082Route = "menu";
    menuButton.innerHTML = '<i>☰</i><b>MENU</b>';
  }
  return menuButton;
}

function arrangeDesktopRail(rail) {
  if (!rail) return;
  const railNav = q('.rail-nav-v060.rail-nav-v082', rail) || q('.rail-nav-v060', rail);
  if (!railNav) return;
  const byRoute = new Map();
  qa('button[data-v082-route]', railNav).forEach(button => {
    byRoute.set(button.dataset.v082Route, button);
  });
  byRoute.set('menu', ensureDesktopMenuButton(railNav));

  const order = [
    'skins', 'gear', 'arsenalV070', 'playerUpgrade',
    'worlds', 'menu', 'bagV082', 'gachaV082',
    'shopV082', 'collectionV082', 'achievements'
  ];
  const labels = {
    skins: copy('Cherry', 'Cherry'),
    gear: copy('Gear', 'Gear'),
    arsenalV070: 'Arsenal',
    playerUpgrade: copy('Upgrade', 'Upgrade'),
    worlds: 'PLAY',
    menu: 'MENU',
    bagV082: 'BAG',
    gachaV082: 'Gacha',
    shopV082: copy('Shop', 'Shop'),
    collectionV082: copy('Collection', 'Collection'),
    achievements: copy('Achievements', 'Achievements')
  };

  order.forEach(route => {
    const button = byRoute.get(route);
    if (!button) return;
    const label = button.querySelector('b');
    if (label && labels[route]) label.textContent = labels[route];
    button.classList.toggle('rail-cta-v0948', route === 'worlds' || route === 'menu');
...
function renderDesktopChrome() {
  ...
  if (upgradeLabel) upgradeLabel.textContent = copy("Upgrade", "Upgrade");
  arrangeDesktopRail(rail);
