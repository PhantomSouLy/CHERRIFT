(() => {
  ...
  const VERSION = "0.9.4.3-desktop-map-input-stability";
  ...
  const MAP_OBJECT_PATHS = Object.freeze({
    ...
    // The optimized asset set has no separate world_rock_4.png.
    w4_rock4:"assets/map/world4/world_rock_3.png",
  ...
  function chapterArt(world, chapter) {
    const artIndex = chapter <= 2 ? 1 : chapter <= 4 ? 2 : 3;
  ...
  function observeUi() {
    ...
    state.observer = new MutationObserver(mutations => {
      const meaningful = mutations.some(mutation => {
        ...
        return !element?.closest?.("#clickPetalBurstV0943,#desktopCurrencyV0943");
      });
      if (!meaningful) return;
      clearTimeout(state.observerTimer);
      state.observerTimer = setTimeout(patchVisibleUi, 45);
    });
  ...
  function start() {
    ...
    window.CHERRIFT_BUGFIX_V0943 = Object.freeze({
      version:VERSION,
      open:safeOpen,
