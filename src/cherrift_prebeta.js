(() => {
  ...
  function patchStorage() {
    ...
    storage.save = value => {
      ...
      if(lastCoins!==null&&current>lastCoins){
        ...
        const bonus=Math.floor(rawGain*Number(titleStats(value).coinGain||0));
        if(bonus>0)value.coins=current+bonus;
        value.economy.lifetimeCoinsEarned=n(value.economy.lifetimeCoinsEarned)+rawGain+bonus;
      }
      lastCoins=n(value.coins);
  ...
  function patchGame() {
    ...
    const start=proto.start; proto.start=async function(...args){ if(!startRunAllowed(this))return false; const result=await start.apply(this,args);if(this.player){const bonus=titleStats(this.save);this.player.damage+=bonus.damage+bonus.allStats;this.player.maxHp+=bonus.maxHp+bonus.allStats;this.player.hp+=bonus.maxHp+bonus.allStats;this.player.speed+=bonus.allStats;if(Number.isFinite(Number(this.player.armor)))this.player.armor+=bonus.allStats;}return result; };
